...
Для быстрого показа проекта запустите приложение и откройте демо-режим:

```text
http://localhost:5000/demo
```

Демо-режим автоматически:
- создаёт HR-пользователя;
- создаёт демонстрационную вакансию Python Backend Developer;
...
- авторизует демонстрационного HR и открывает общий отчёт.
...
2. Нажать `Демо` и показать общий отчёт по кандидатам.
3. Открыть отчёт сильного кандидата и показать оценку по вопросам.
4. Вернуться в `Панель HR`, показать ссылку кандидата и статистику.
5. Открыть `Создать вакансию`, вставить пример и показать генерацию нового теста.
...
- SQLite fallback рассчитан на локальный запуск и демонстрационный режим, а PostgreSQL остаётся основным вариантом для полноценной среды;
